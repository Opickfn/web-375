<div>
    ...
    <div class="pm-stat" style="max-width:340px;margin-bottom:1.5rem;">
        ...
        <div>
            <div class="pm-stat-value">{{ number_format($totalPoints) }}</div>
    ...
    <div class="pm-toolbar">
        ...
        <div class="pm-filters">
            ...
            <select wire:model.live="perPage" class="pm-select">
    ...
    <div class="pm-card">
        ...
        <div class="pm-table-wrap">
            <table class="pm-table">
                <thead>
                    <tr>
                        <th class="pm-th-sort" wire:click="sort('created_at')">Tanggal
                            @include('components.sort-icon', ['col' => 'created_at', 'sortBy' => $sortBy, 'sortDir' => $sortDir])
                        </th>
                        <th>Kode Laporan</th>
                        <th>Deskripsi</th>
                        <th>Tipe</th>
                        <th style="text-align:right;" class="pm-th-sort" wire:click="sort('amount')">Poin
                            @include('components.sort-icon', ['col' => 'amount', 'sortBy' => $sortBy, 'sortDir' => $sortDir])
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($points as $point)
                        <tr wire:key="pt-{{ $point->id }}">
                            ...
                            <td>
                                @if($point->type == 'submit')
                                    <span class="pm-badge pm-badge-accent">{{ ucfirst($point->type) }}</span>
                                @elseif($point->type == 'approved')
                                    <span class="pm-badge pm-badge-success">{{ ucfirst($point->type) }}</span>
                                @elseif($point->type == 'bonus')
                                    <span class="pm-badge pm-badge-gold">{{ ucfirst($point->type) }}</span>
                                @elseif($point->type == 'rejected')
                                    <span class="pm-badge pm-badge-danger">{{ ucfirst($point->type) }}</span>
                                @else
                                    <span class="pm-badge pm-badge-neutral">{{ ucfirst($point->type) }}</span>
                                @endif
                            </td>
